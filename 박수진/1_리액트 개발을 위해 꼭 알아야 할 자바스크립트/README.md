...
객체 타입은 참조값 저장방식으로, 값 비교시 예상과 다른 결과일 때가 있다.
#### 원시 타입
...
- prototype 에 함수가 등록되어 프로토타입 체이닝 ( 자기자신 부터 최상위 객체 까지 프로토타입을 타고 가며 메서드를 호출)을 통해 인스턴스에서 메서드 사용이 가능하다. 
...
![[Pasted image 20231218151946.png]]
...
window.requestAnimationFrame(()=>{console.log('animate')}) 
